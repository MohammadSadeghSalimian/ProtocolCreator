...
public class DriftSegment(double start, double end,double unsignedStep)
{
    ...
    public LoadingPhase LoadingPhase { get; } = Math.Abs(start) <= Math.Abs(end) ? LoadingPhase.Loading : LoadingPhase.Unloading; // Determine loading phase based on drift values
}