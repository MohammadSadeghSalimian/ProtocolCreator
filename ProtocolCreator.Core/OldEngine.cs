...
public class OldEngine(IReadOnlyList<DriftSegment> driftSegments, AnalysisInformation info):BaseEngine(driftSegments, info)
{
        ...
        foreach (var item in DriftSegments)
        {
            ...
            switch (item.CycleState)
            {
                case CycleState.PL:
                    ...
                    SetPositiveLoading(item, step, id, ref cycle, dy, ref peakPositive, ref isExperiencedYield, co, dEff, ref currentElongation);
                    break;
                case CycleState.PU:
                   
                    SetNegativeLoading(item, step, dy, id, ref cycle, co, dEff, ref currentElongation, ref isExperiencedYield);
                    break;
                case CycleState.NL:
                    SetNegativeLoading(item, step, id, ref cycle, dy, ref peakNegative, ref isExperiencedYield, co, dEff, ref currentElongation);
                    break;
                case CycleState.NU:
                    SetNegativeUnloading(item, step, dy, id, ref cycle, co, dEff, ref currentElongation, ref isExperiencedYield);
    ...
    private void SetNegativeUnloading(DriftSegment item, double step, double dy, int id, ref double cycle,
        CoefficientContainer co, double dEff, ref double currentElongation, ref bool isExperiencedYield)
    {
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            var currentCoefficient = co.Negative.PlasticUnloading;
            double slope;
            int repeat;
    ...
    private void SetNegativeLoading(DriftSegment item, double step, int id,ref double cycle, double dy, ref double peakNegative,
    ...
    private void SetNegativeLoading(DriftSegment item, double step, double dy, int id, ref double cycle,
        CoefficientContainer co, double dEff, ref double currentElongation, ref bool isExperiencedYield)
    {
        var deltaValues = MathExtension.Arrange(item.Start, item.End, -step);
        var futureDrift = item.Start - dy;
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            var currentCoefficient = co.Positive.PlasticUnloading;
            double slope;
            int repeat;
            bool isInResidualArea;
            if (b >= futureDrift)
    ...
    private void SetPositiveLoading(DriftSegment item, double step, int id,ref double cycle, double dy, ref double peakPositive,
        ref bool isExperiencedYield, CoefficientContainer co, double dEff, ref double currentElongation)
    {
        var deltaValues = MathExtension.Arrange(item.Start, item.End, step);
        var n = deltaValues.Length;
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            if (a < dy && b <= dy)
            {
                currentCoefficient = co.Positive.ElasticLoading;
                isInPlasticArea = false;
            }
            else
            {
