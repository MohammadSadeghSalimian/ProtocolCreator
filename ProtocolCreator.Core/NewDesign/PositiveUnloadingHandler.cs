...
internal sealed class PositiveUnloadingHandler(AnalysisInformation info, ICoefficientProvider coefs, List<Delta> sink)
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (b < future && a > future)
                throw new ArgumentException(
                    $"The future drift ({future}) is placed between the steps. This is not allowed.");

            var coef = Coefs.GetUnloadingPlastic(Direction.Positive);
